...
> 当前仓库已进入 Web Pipeline 第一阶段准备：Jenkins 镜像已从基础版升级为 `jenkins:lts-jdk21-docker-kubectl-amd64`，并挂载 ECS Docker socket，用于先跑通 `web` 模块 CI/CD 闭环。详细步骤见 `docs/cicd/JENKINS_WEB_PIPELINE_DOCKER_DEPLOYMENT_GUIDE.md`。
...
| 暴露方式 | 第一版建议 port-forward | 先保证本地可访问，Jenkins 不建议公网裸露。 |
